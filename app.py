...
#         # Step 2: Build LR & XGB input vectors
...
#         # Step 4: Build vertical comparison tables
#         lr_display = pd.DataFrame({
#             "Feature": lr_vector.columns,
#             "Raw Value": [validated_raw[f] for f in lr_vector.columns],
#             "Model Ready (WOE)": lr_vector.iloc[0].values
#         })

#         xgb_display = pd.DataFrame({
#             "Feature": xgb_vector.columns,
#             "Raw Value": [validated_raw.get(f, None) for f in xgb_vector.columns],
...
#             st.metric("LR Predicted PD (%)", f"{lr_pd} %")

#         with col2:
#             st.subheader("📗 XGBoost")
#             st.dataframe(xgb_display, use_container_width=True)
#             st.metric("XGB Predicted PD (%)", f"{xgb_pd} %")
...
if generate_btn:
    ...
    try:
        ...
        lr_decision_df = decision_engine.evaluate(lr_pd, model_name="LR")
        xgb_decision_df = decision_engine.evaluate(xgb_pd, model_name="XGB")
        ...
        col1.metric("PD (%)", f"{lr_pd:.2f}%")
        col2.metric("Credit Score", int(lr_decision_df["credit_score"][0]))
        col3.metric("Rating Band", lr_decision_df["rating_band"][0])

        decision = lr_decision_df["decision"][0]

        if "Decline" in decision:
            col4.error(decision)
        elif "Review" in decision:
            col4.warning(decision)
        else:
        ...
        col1.metric("PD (%)", f"{xgb_pd:.2f}%")
        col2.metric("Credit Score", int(xgb_decision_df["credit_score"][0]))
        col3.metric("Rating Band", xgb_decision_df["rating_band"][0])

        decision = xgb_decision_df["decision"][0]
        ...
        # =====================================================
        # KEEP INPUT VECTOR TABLES (AS YOU REQUESTED)
        # =====================================================

        st.markdown("### Logistic Regression – Model Ready (WOE)")
        st.dataframe(lr_vector, use_container_width=True)

        st.markdown("### XGBoost – Model Ready (Encoded)")
